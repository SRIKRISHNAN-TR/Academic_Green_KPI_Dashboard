...
export default function Reports() {
  ...
  const handleGenerate = async (format: "pdf" | "docx") => {
    ...
    if (data.records.length === 0) {
      toast({ title: "No data available", description: "No records found for the selected filters.", variant: "destructive" });
  ...
  if (!isAuthenticated) {
    return <PublicLayout title="Reports">{pageContent}</PublicLayout>;
  }

  return <DashboardLayout title="Reports" requireAuth={false}>{pageContent}</DashboardLayout>;
}