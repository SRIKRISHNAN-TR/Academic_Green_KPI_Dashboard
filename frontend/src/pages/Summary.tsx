...
import { useDashboardSummary, energyHooks } from "@/hooks/useApi";
...
export default function Summary() {
  ...
  const { data: energyRaw } = energyHooks.useAll({ year: new Date().getFullYear() });
  ...
  const chartData = (energyRaw || []).map((d) => ({
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <KPICard
          ...
          value={summary ? summary.energy.ytd.actual.toLocaleString() : "0"}
          unit="kWh"
          target={summary ? summary.energy.ytd.target.toLocaleString() : "0"}
          status={summary ? mapStatus(summary.energy.ytd.status) : "success"}
          description={summary && summary.energy.ytd.target > 0 ? `${Math.round(((summary.energy.ytd.target - summary.energy.ytd.actual) / summary.energy.ytd.target) * 100)}% below annual target` : ""}
          icon={<IconBolt className="size-5" />}
          variant="energy"
        />
        <KPICard
          title="Water (YTD)"
          value={summary ? summary.water.ytd.actual.toLocaleString() : "0"}
          unit="m³"
          target={summary ? summary.water.ytd.target.toLocaleString() : "0"}
          status={summary ? mapStatus(summary.water.ytd.status) : "success"}
          description={summary && summary.water.ytd.target > 0 ? `${Math.round(((summary.water.ytd.target - summary.water.ytd.actual) / summary.water.ytd.target) * 100)}% below annual target` : ""}
          icon={<IconDroplet className="size-5" />}
          variant="water"
        />
        <KPICard
          title="Waste Diverted (YTD)"
          value={summary ? summary.waste.ytd.actual.toLocaleString() : "0"}
          unit="%"
          target={summary ? summary.waste.ytd.target.toLocaleString() : "0"}
          status={summary ? mapStatus(summary.waste.ytd.status) : "success"}
          description="Year-to-date diversion rate"
          icon={<IconRecycle className="size-5" />}
          variant="waste"
        />
      </div>
        ...
        <div className="lg:col-span-2">
          {chartData.length > 0 ? (
            <TrendChart
              ...
              description="Comparison across months"
              data={chartData}
              type="bar"
            />
          ) : (
  ...
  if (!isAuthenticated) {
    return <PublicLayout title="Summary">{pageContent}</PublicLayout>;
  }
