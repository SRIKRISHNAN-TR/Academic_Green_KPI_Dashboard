...
export default function DataEntry() {
  ...
  const recentSubmissions = [
    ...(recentEnergy || []).slice(0, 2).map((d) => ({
      ...
      location: d.location || "—", submittedBy: user?.name || "Anonymous", date: new Date(d.createdAt).toLocaleDateString(),
    })),
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Recent data entry submissions</CardDescription>
        </CardHeader>
        <CardContent>
  ...
  if (!isAuthenticated) {
    return <PublicLayout title="Data Entry">{pageContent}</PublicLayout>;
  }

  return <DashboardLayout title="Data Entry" requireAuth={false}>{pageContent}</DashboardLayout>;
}